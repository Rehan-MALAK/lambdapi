...
type execution_mode =
  ...
  | Uglyfy   (** Parse and uglyfy-print the files. *)
...
let handle_file : string -> unit = fun fname ->
  try
    ...
    | Uglyfy  -> Ugly.uglyfy (Compile.parse_file fname)
    | Normal    ->
...
let spec =
  ...
  let spec = Arg.align
    ...
    ; ( "--type-lhs"
      , Arg.Unit (fun _ -> Sr.forbids_untypable_lhs := true)
    ...
    ; ( "--uglyfy"
      , Arg.Unit (fun _ -> mode := Uglyfy; set_default_verbose 0)
      , " Parse input files and uglyfy-print them (in the old syntax)" )
    ; ( "--timeout"
      , Arg.Int set_timeout
