...
let print_domains : bool Timed.ref = Timed.ref true
let print_implicits : bool Timed.ref = Timed.ref true
let print_meta_type : bool Timed.ref = Timed.ref true
let print_contexts : bool Timed.ref = Timed.ref true

let put_console_debug_on () =
  let open Timed in
  print_domains := !Print.print_domains;
  print_implicits := !Print.print_implicits;
  print_meta_type := !Print.print_meta_type;
  print_contexts := !Print.print_contexts;
  Print.print_domains := true;
  Print.print_implicits := true;
  Print.print_meta_type := true;
  Print.print_contexts := true

let put_console_debug_default () =
  let open Timed in
  Print.print_domains := !print_domains;
  Print.print_implicits := !print_implicits;
  Print.print_meta_type := !print_meta_type;
  Print.print_contexts := !print_contexts

let print_ctxt fmt (a_c : Terms.ctxt) =
  let print_a_c fmt (tvar,t1,t2o) =
    fprintf fmt "\ntvar=%a term=%a termoption=%a" print_bindlib_var tvar print_term t1 (print_option print_term) t2o
  in fprintf fmt "%a" (print_list comma print_a_c) a_c

let print_goal fmt (a_g : Proof.Goal.t) =
  put_console_debug_on ();
  begin
    match a_g with
    | Typ(a_g) ->
      fprintf fmt "#####TYPE_GOAL#####%a:%a\nmeta=%a\nhyps=?\ntype=%a" Print.pp_meta a_g.goal_meta Print.pp_term a_g.goal_type print_meta a_g.goal_meta print_term a_g.goal_type
    | Unif(ctxt,t1,t2) ->
      fprintf fmt "#####UNIF_GOAL#####%a\nctxt:%a      \n%a\n=?\n%a" Print.pp_typing (ctxt,t1,t2) print_ctxt ctxt print_term t1 print_term t2
  end;
