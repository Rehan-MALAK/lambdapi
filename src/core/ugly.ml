...
let pp_arg_ident : ident option pp = fun oc id ->
  ...
  | None     -> let none_id = "none" ^ string_of_int (no_arg_counter ()) in
    Format.pp_print_string oc none_id
...
let pp_prop : Terms.prop pp = fun oc p ->
  match p with
  | Defin -> ()
(*   | Const -> Format.pp_print_string oc "constant " TODO*)
  | Const -> Format.pp_print_string oc ""
  | Injec -> Format.pp_print_string oc "injective "

let[@warning "-33-39-51"] pp_p_patt : p_term pp = fun oc t ->
  let open Parser in
  let (pvs_lhs,_) = Scope.patt_vars t in
  let pvs,_ = List.split pvs_lhs in
  let out fmt = Format.fprintf oc fmt in
  let pp _p t =
    let pp_string_list = List.pp Format.pp_print_string "," in
    out "%a" pp_string_list (List.rev t)
  in
  pp oc pvs
...
let rec pp_p_term : p_term pp = fun oc t ->
  ...
  let rec pp p _ t =
    ...
    match (t.elt, p) with
(*     | (P_Type            , _    ) -> out "TYPE" TODO*)
    | (P_Type            , _    ) -> out "Type"
    | (P_Iden(qid, false), _    ) -> out "%a" pp_qident qid
    | (P_Iden(qid, true ), _    ) -> out "@%a" pp_qident qid
    | (P_Wild            , _    ) -> out "_"
    | (P_Meta(x,ar)      , _    ) -> out "?%a%a" pp_ident x pp_env ar
(*     | (P_Patt(None   ,ar), _    ) -> out "&_%a" pp_env ar TODO*)
    | (P_Patt(None   ,ar), _    ) -> out "%a" pp_env ar
(*     | (P_Patt(Some(x),ar), _    ) -> out "&%a%a" pp_ident x pp_env ar TODO*)
    | (P_Patt(Some(x),ar), _    ) -> out "%a%a" pp_ident x pp_env ar
    | (P_Appl(t,u)       , PAppl)
    | (P_Appl(t,u)       , PFunc) -> out "%a@ %a" pp_appl t pp_atom u
(*     | (P_Impl(a,b)       , PFunc) -> out "%a@ ⇒ %a" pp_appl a pp_func b TODO*)
    | (P_Impl(a,b)       , PFunc) -> out "%a@ -> %a" pp_appl a pp_func b
(*     | (P_Abst(args,t)    , PFunc) -> out "λ%a,@ %a" pp_p_args args pp_func t TODO*)
    | (P_Abst(args,t)    , PFunc) -> begin match args with
        | []     -> out "%a" pp_func t
        | hd::tl -> let elt = P_Abst(tl,t) in
          let eltpos = {elt;pos=t.pos;} in
          out "%a@ => %a" pp_p_args_noparen [hd] pp_func eltpos
      end
(*     | (P_Prod(args,b)    , PFunc) -> out "∀%a,@ %a" pp_p_args args pp_func b TODO*)
  ...
  let rec pp_toplevel _ t =
    match t.elt with
(*     | P_Abst(args,t)     -> out "λ%a,@ %a" pp_p_args args pp_toplevel t TODO*)
    | P_Abst(args,t)     ->  begin match args with
        | []     -> out "%a" pp_toplevel t
        | hd::tl -> let elt = P_Abst(tl,t) in
          let eltpos = {elt;pos=t.pos;} in
          out "%a@ => %a" pp_p_args_noparen [hd] pp_toplevel eltpos
      end
(*     | P_Prod(args,b)     -> out "∀%a,@ %a" pp_p_args args pp_toplevel b TODO*)
    | P_Prod(args,b)     -> out "%a@ -> %a" pp_p_args args pp_toplevel b
(*     | P_Impl(a,b)        -> out "%a@ ⇒ %a" (pp PAppl) a pp_toplevel b TODO*)
...
and pp_p_arg : p_arg pp = fun oc (ids,ao,b) ->
  let pp_id oc2 id =
  match (ao,b) with
  | (None   , false) -> Format.fprintf oc2 "%a" pp_arg_ident id
  | (None   , true ) -> Format.fprintf oc2 "{%a}" pp_arg_ident id
  | (Some(a), false) -> Format.fprintf oc2 "(%a : (%a))" pp_arg_ident id pp_p_term a
  | (Some(a), true ) -> Format.fprintf oc2 "{%a : %a}" pp_arg_ident id pp_p_term a
  in
  Format.fprintf oc "%a" (List.pp pp_id " ") ids
(*
and pp_p_arg_noparen : p_arg pp = fun oc (ids,ao,b) ->
  let pp_ids = List.pp pp_arg_ident " " in
  match (ao,b) with
  | (None   , false) -> Format.fprintf oc "%a" pp_ids ids
  | (None   , true ) -> Format.fprintf oc "{%a}" pp_ids ids
...
and pp_p_args_noparen : p_arg list pp = fun oc ->
  List.iter (Format.fprintf oc " %a" pp_p_arg_noparen)

let pp_p_rule : p_rule pp = fun oc r ->
  let (lhs, rhs) = r.elt in
(*   Format.fprintf oc "@[<hov 3>rule %a@ → %a@]@?" pp_p_term lhs pp_p_term rhs *)
  Format.fprintf oc "@[<hov 3>[%a] %a@ --> %a@]@?" pp_p_patt lhs pp_p_term lhs pp_p_term rhs
...
(*   | P_assert_conv(t,u)   -> out "%a@ ≡ %a" pp_p_term t pp_p_term u TODO*)
...
(*       out "assertnot %a" pp_p_assertion asrt TODO*)
      out "#ASSERTNOT %a" pp_p_assertion asrt
(*   | P_query_assert(false, asrt)           -> *)
  | P_query_assert(false, asrt)           ->
(*       out "assert %a" pp_p_assertion asrt TODO*)
...
(*       out "#ASSERT n0 == n0" *)
  ...
  | P_query_infer(t, _)                   ->
(*       out "@[<hov 4>type %a@]" pp_p_term t TODO*)
      out "@[<hov 4>#TYPE %a@]" pp_p_term t
  | P_query_normalize(t, _)               ->
(*       out "@[<hov 2>compute@ %a@]" pp_p_term t TODO*)
      out "@[<hov 2>#EVAL@ %a@]" pp_p_term t
(*       out "@[<hov 2>#EVAL@ n0@]" *)
  | P_query_prover(s)                     ->
      out "set prover \"%s\"" s
...
(*       out "require%s %a" op (List.pp (pp_path cmd.pos) " ") ps TODO*)
      out "#REQUIRE%s %a" op (List.pp (pp_path cmd.pos) " ") ps
  | P_require_as(p,i)               ->
(*       out "require %a as %a" (pp_path cmd.pos) p (pp_path_elt i.pos) i.elt TODO*)
      out "#REQUIRE %a as %a" (pp_path cmd.pos) p (pp_path_elt i.pos) i.elt
  | P_open(ps)                      ->
      List.iter (out "open %a" (pp_path cmd.pos)) ps
  | P_symbol(e,p,s,args,a)    ->
(*       out "@[<hov 2>%a%asymbol %a" pp_expo e pp_prop p pp_ident s; TODO*)
    let prop = match p with | Defin | Injec -> "def" | Const -> "" in
      out "@[<hov 2>%a%a%s %a" pp_expo e pp_prop p prop pp_ident s;
      List.iter (out " %a" pp_p_arg) args;
      out " :@ @[<hov>%a@]" pp_p_term a
  | P_rules(rs)                     ->
      out "%a" (List.pp pp_p_rule "\n") rs
  | P_definition(e,_,s,args,ao,t)   ->
(*       out "@[<hov 2>%adefinition %a" pp_expo e pp_ident s; TODO*)
      out "@[<hov 2>%adef %a" pp_expo e pp_ident s;
      List.iter (out " %a" pp_p_arg) args;
      Option.iter (out " :@ @[<hov>%a@]" pp_p_term) ao;
(*       out " ≔@ @[<hov>%a@]@]" pp_p_term t TODO*)
