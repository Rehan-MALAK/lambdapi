...
let rec print_term fmt (a_pos : Terms.term) =
  ...
  | Symb(sym,_(*pp_hint*)) -> fprintf fmt "Symb(%a)" print_sym sym (*print_pp_hint pp_hint*)
...
let print_rule fmt (a_rule : Terms.rule) =
  let print_name_arity fmt (name,arity) = fprintf fmt "\"%s\" arity %d" name arity in
(* NOT ALLOWED IN OCAMLDEBUG
  let (_,rhs) = Bindlib.unmbind a_rule.rhs in
  fprintf fmt "@\nlhs :@\n            %a@\nrhs mbinder:@\n            %a@\nrhs :@\n            %a@\narity :@\n          %d@\nvars bound rhs :@\n         %a" (print_list comma print_term) a_rule.lhs print_bindlib_mbinder a_rule.rhs print_term rhs a_rule.arity (print_array print_name_arity) a_rule.vars
*)
  fprintf fmt "@\nlhs :@\n            %a@\nrhs mbinder:@\n            %a@\narity :@\n          %d@\nnvars bound rhs :@\n         %a" (print_list comma print_term) a_rule.lhs print_bindlib_mbinder a_rule.rhs a_rule.arity (print_array print_name_arity) a_rule.vars
