...
(* NOT ALLOWED IN OCAMLDEBUG
  ...
  | None -> None
  in
  fprintf fmt "\"%a\",arity:%d,value binder:%a,value:%a" (print_option_or_default "?" pp_print_string) a_meta.meta_name a_meta.meta_arity (print_option print_bindlib_mbinder) a_meta_deref (print_option print_term) a_meta_deref_deoption_unbinded
*)
...
let print_goal fmt (a_g : Proof.Goal.t) =
  match a_g with
  | Typ(a_g) ->
  fprintf fmt "#####TYPE_GOAL#####\nmeta=%a\nhyps=?\ntype=%a" print_meta a_g.goal_meta print_term a_g.goal_type
  | Unif(ctxt,t1,t2) ->
  fprintf fmt "#####UNIF_GOAL#####\nctxt:%a      \n%a\n=?\n%a" print_ctxt ctxt print_term t1 print_term t2
